...
| egress proof | `./egress/proof/run.sh` | Docker |
| live smoke | `AGENTPLANE_URL=https://… AGENTPLANE_TOKEN=apl_… ./test/smoke-live.sh` | deployed agentplane; costs one model turn |

Run the first two before merging changes to `hand/` or `egress/`; run the
live smoke before handing the system to testers or after a redeploy.
